...
const BusinessCardListItem = ({
  ...
  const titleColor = status === "invalid" ? "#D22B2B" : "white";
  const subtitleColor = status === "invalid" ? "#D22B2B" : "#666";
  const subtitleText =
    status === "invalid"
      ? `Invalid - ${role}@${company}`
      : `${role}@${company}`;

  const showUndoButton = status === "invalid" && !isDeleted;
    ...
    <ListItem.Swipeable
      ...
      rightContent={(action) => (
        <Button
          ...
          type="clear"
          icon={{ name: showUndoButton ? "undo" : "delete-outline" }}
          onPress={showUndoButton ? () => onUndo(id) : () => onDelete(id)}
        />
      )}
    >
      <TouchableOpacity
